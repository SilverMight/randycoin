Randycoin Core version *0.15.1* is now available from:

  <https://download.randycoin.org/randycoin-0.15.1/>
...
A number of changes to the way Randycoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 rudys per kB (0.00001 RND). This is to prepare for dropping the minimum transaction fee to 0.1 rudys per kB (0.0001 RND) in the next release.
