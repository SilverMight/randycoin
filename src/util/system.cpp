...
const char * const BITCOIN_CONF_FILENAME = "randycoin.conf";
...
#else
    const char* pszModule = "randycoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Randycoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Randycoin";
#else
    // Unix
    return pathRet / ".randycoin";
#endif
#endif
}
